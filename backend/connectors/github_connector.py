...
def _paginate(path: str, params: dict) -> list[dict]:
    ...
    query = dict(params)
    while url:
        resp = _request("GET", url, params=query)
...
def _list_recent_issues(owner: str, repo: str, since_iso: str) -> list[dict]:
...
def _list_comments(owner: str, repo: str, issue_number: int, since_iso: str) -> list[dict]:
    comments = _paginate(
...
def fetch_github_documents() -> list[dict]:
    ...
    for repo_full in repos:
        ...
        if not repo:
            ...
            continue
        issues = _list_recent_issues(owner, repo, since_iso)
        for issue in issues:
            comments = _list_comments(owner, repo, issue["number"], since_iso)
